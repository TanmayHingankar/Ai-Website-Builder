...
export const generateWebsite = async (req, res) => {
    try {
        ...
        if (user.credits < CREDIT_COSTS.generate) {
        ...
        user.credits = user.credits - CREDIT_COSTS.generate
        await user.save()
        await Transaction.create({
            user: user._id,
            type: "credit_spend",
            amount: CREDIT_COSTS.generate,
            reason: "generate",
            description: "Website generation",
            balanceAfter: user.credits,
            metadata: { websiteId: website._id }
        })
...
export const changes = async (req, res) => {
    try {
        ...
        if (user.credits < CREDIT_COSTS.update) {
            return res.status(400).json({ message: "you have not enough credits to generate a webiste" })
        }
...
`
        ...
        await website.save()
        user.credits = user.credits - CREDIT_COSTS.update
        await user.save()
        await Transaction.create({
            user: user._id,
            type: "credit_spend",
            amount: CREDIT_COSTS.update,
            reason: "update",
...
export const deploy=async (req,res)=>{
    try {
        ...
        if (user.credits < CREDIT_COSTS.deploy) {
            return res.status(400).json({ message: "you have not enough credits to deploy a website" })
        }
